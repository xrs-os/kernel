...
def objcopy():
    exe = (shutil.which("rust-objcopy") or shutil.which("llvm-objcopy"))
...
def build_kernel(release, arch, target_name, build_dir):
    ...
    cargobuild = [
        "cargo", "-Zunstable-options", "-Zconfig-include", "--config",
        "include=\"kernel_cargo_config.toml\"", "build"
    ]
    if release:
        cargobuild += ["--release"]
    ...
    kernel = "target/{}/{}/kernel".format(target,
...
def build_init_proc(release, arch, target_name, build_dir):
    ...
    cargobuild += ["--target", target]
    run(cargobuild, verbose=True)
    src = "target/{}/{}/init_proc".format(target,
                                          "release" if release else "debug")
...
def cmd_qemu(args):
    kernel_bin_path = build("kernel.bin", args.build_dir, args.release,
                            args.arch)
    initfs_img_path = build("initfs.img", args.build_dir, args.release,
                            args.arch)
    cmd = [
...
def main():
    ...
    parser.add_argument("--build_dir",
                        default="./build",
                        help="Copy final artifacts to this directory")

    subparser = parser.add_subparsers(title="subcommands")

    buildcmd = subparser.add_parser("build", help="Build the specified target")
    buildcmd.add_argument("target",
                          default="kernel.bin",
                          help="Specified target")
    buildcmd.set_defaults(func=cmd_build)

    qemucmd = subparser.add_parser("qemu",
                                   help="Build the kernel and run on qemu")
    qemucmd.add_argument("--smp",
                         type=int,
                         help="Specify number of CPU cores",
                         default=4)
    qemucmd.add_argument("--ram",
                         "-m",
                         dest="ram",
                         help="Specify size of ram",
                         default="4G")
    qemucmd.add_argument("--gdb",
                         action="store_true",
                         help="Start gdbserver",
                         default=False)
    qemucmd.set_defaults(func=cmd_qemu)

    cleancmd = subparser.add_parser(
        "clean",
        help="Remove artifacts that `bootstrap.py` has generated in the past")
